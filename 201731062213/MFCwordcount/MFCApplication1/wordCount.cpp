...
void wordCount::openFile(string filename)
{
	filepro.fileOpen(filename);
}

void wordCount::outPutToStr()
{
	outputStr[0] = "characters:"+ to_string( cntchars)+ "\r\n";
	outputStr[1] = "Words:" + to_string(cntwords) + "\r\n";
	outputStr[3] = "Lines:" + to_string(cntlines) + "\r\n";
	int i = 4;
	map<string, int>::iterator it;
	for (it = freq.begin(); it != freq.end(); it++)
	{
		outputStr[i]  =  "<" + it->first + ">:" + to_string( it->second ) + "\r\n";
		i++;
	}
	t = i;
}
